...
// MountConsensuslessStores mounts all in-memory KVStores with the BaseApp's internal
// commit multi-store.
func (app *BaseApp) MountConsensuslessStores(keys map[string]*storetypes.ConsensuslessStoreKey) {
	skeys := maps.Keys(keys)
	sort.Strings(skeys)
	for _, key := range skeys {
		clKey := keys[key]
...
func (app *BaseApp) runTx(mode execMode, txBytes []byte) (gInfo sdk.GasInfo, result *sdk.Result, anteEvents []abci.Event, err error) {
	...
	if app.anteHandler != nil {
		...
		fmt.Println("--baseapp.anteHandler--", err, ctx.GasMeter().Limit(), ctx.GasMeter().GasConsumed(), "--newCtx--", newCtx.GasMeter().Limit(), newCtx.GasMeter().GasConsumed())
		if !newCtx.IsZero() {
